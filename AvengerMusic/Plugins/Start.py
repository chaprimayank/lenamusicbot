...
from AvengerMusic import ASSIDS, BOT_ID, MUSIC_BOT_NAME, OWNER_ID, SUDOERS, app
from AvengerMusic import boottime as bot_start_time
from AvengerMusic import db, random_assistant
from AvengerMusic.Core.PyTgCalls import Avenger
from AvengerMusic.Database import (add_nonadmin_chat, add_served_chat,
...
from AvengerMusic.Decorators.admins import ActualAdminCB
from AvengerMusic.Decorators.permission import PermissionCheck
from AvengerMusic.Inline import (custommarkup, dashmarkup, setting_markup,
                          setting_markup2, start_pannel, usermarkup, volmarkup)
from AvengerMusic.Utilities.assistant import get_assistant_details
from AvengerMusic.Utilities.ping import get_readable_time
    ...
    for member in message.new_chat_members:
        try:
            if member.id == BOT_ID:
                ...
                out = start_pannel()
                await message.reply_text(
                    f"Promote me as administrator in your group otherwise I will not function properly.",
...
async def start_markup_check(_, CallbackQuery):
    ...
    if command == "LV":
        ...
        volume = 25
        try:
            await Avenger.pytgcalls.change_volume_call(c_id, volume)
    ...
    if command == "DIT":
        ...
        await CallbackQuery.answer(
            f"Avenger Disk Usage: {diske}%", show_alert=True
        )
